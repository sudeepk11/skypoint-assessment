"""Seed script — populates default users and sample jobs on first startup.
...
def run() -> None:
    ...
    try:
        ...
        for job_data in sample_jobs:
            ...
            if not existing:
                db.add(Job(created_by=hr_user.id, **job_data))

        db.commit()
        logger.info("Seed complete — %d jobs checked.", len(sample_jobs))
    finally:
        db.close()
