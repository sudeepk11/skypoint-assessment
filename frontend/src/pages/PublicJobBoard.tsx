...
import { Search, MapPin, Clock, DollarSign, Briefcase, Building2, X } from 'lucide-react';
...
function matchesSalaryFilter(salaryRange: string | undefined, filter: SalaryFilter): boolean {
  ...
  // Detect INR vs USD by checking for ₹ symbol or large numbers
  const isINR = salaryRange?.includes('₹') ?? false;
  if (isINR) {
    // Values in lakhs: entry 0–15L, mid 15L–30L, senior 30L+
    const maxL = max / 100000;
    if (filter === 'entry') return maxL <= 1500000 / 100000; // up to 15L
...
const PublicJobBoard: React.FC = () => {
  ...
  const filteredJobs = jobList.filter(job => {
    ...
    return matchesSkills && matchesLocation && matchesSalary;
  });
  ...
  return (
    <div className="min-h-screen bg-surface flex flex-col">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
        {/* Page header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Browse Open Jobs</h1>
          <p className="text-gray-500">
            Discover your next opportunity from {filteredJobs.length} open position{filteredJobs.length !== 1 ? 's' : ''}
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4 mb-6 space-y-3">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent"
            />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative">
              ...
              <input
                ...
                className="pl-8 pr-3 py-1.5 border border-gray-200 rounded-lg text-xs bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent w-36"
              />
            </div>
        ...
        {loading ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredJobs.map((job) => (
              <div
                key={job.id}
                className="bg-white rounded-xl border border-gray-100 p-5 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 flex flex-col"
              >
                {/* Type badge */}
                <div className="mb-3">
                  <span
                ...
                <div className="mt-auto">
                  <button
                    ...
                    className="block w-full text-center text-sm font-semibold py-2 rounded-lg bg-accent text-white hover:bg-blue-700 transition-colors"
                  >
                    View Details
                  </button>
                </div>
